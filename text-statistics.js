...
(function(glob) {
	...
	// start added by ohNODEyoudidnt 1-21-17

	TextStatistics.prototype.wordsWithOneSyllable = function(text, countProperNouns) {
		text = text ? cleanText(text) : this.text;
		...
		text.split(/\s+/).forEach(function(word) {
			...
			if (!word.match(/^[A-Z]/) || countProperNouns) {
				if (self.syllableCount(word) == 1) longWordCount ++;
			}
		});

		return longWordCount/this.wordCount(text);
	};

	TextStatistics.prototype.wordsWithTwoSyllables = function(text, countProperNouns) {
		text = text ? cleanText(text) : this.text;
		var longWordCount = 0, self = this;
	...
	TextStatistics.prototype.wordsWithThreeSyllables = function(text, countProperNouns) {
		...
		text.split(/\s+/).forEach(function(word) {
			...
			// Defaults to true.
			if (!word.match(/^[A-Z]/) || countProperNouns) {
				if (self.syllableCount(word) == 3) longWordCount ++;
			}
		});
	...
	// end added by ohNODEyoudidnt 1-21-17

	TextStatistics.prototype.wordsWithMoreThanThreeSyllables = function(text, countProperNouns) {
		text = text ? cleanText(text) : this.text;
		var longWordCount = 0, self = this;
